...
const nameCleanupWords = [
    ...
    'shri', 'smt', 'md', 'sh.', 'm/s', 'karta', 'huf' // Added from previous iteration
];
...
async function getGeminiResponse(prompt) {
    ...
    if (!apiKey) {
        return { text: null, error: "Gemini API key not set in Vercel environment variables.", status: 403 }; // ADDED status 403
    }

    // NOTE: Using gemini-2.5-flash for enhanced reasoning and speed + Tool use
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`;

    // --- ENHANCEMENT: Tool calling for external web search ---
    const requestBody = {
    ...
    try {
        ...
        if (response.status !== 200) {
            ...
            return { text: null, error: errorMessage, status: response.status }; // Return actual API status
        }
        ...
        if (result.candidates && result.candidates.length > 0) {
            ...
            console.error(errorMessage);
            return { text: null, error: errorMessage, status: 500 };
        }
    } catch (e) {
...
2.  **External Knowledge & Verification:** Use your advanced knowledge and **Google Search Tool** to verify missing or suspicious components. If only a major landmark and city are provided, use the search to infer a potential locality or PIN code. If a company name is present (e.g., 'Stahl india company' or 'Metal morphosis pvt Ltd'), search for and use its official, full name (e.g., 'Stahl India Pvt. Ltd.').
...
4.  **Formatting:** Be strict about ensuring the output is a valid, single, and complete address for shipping. Remove duplicate components.
...
14. **"FormattedAddress"**: **CRITICAL FORMATTING INSTRUCTION:** The final, clean, human-readable, shipping-ready address string. It MUST strictly follow this order: **[Official Company Name, if found], [H.no./Flat No.], [Floor], [Street/Colony], [Locality/Village], [P.O. from official data], [Tehsil from official data], [District from official data]**. **DO NOT include the State or PIN in this string.** Use commas to separate logical components.
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (geminiResult.error || !geminiResult.text) {
             // UPDATED: Use the status returned by getGeminiResponse (403 if key is missing)
            const status = geminiResult.status || 500;
            return res.status(status).json({ 
                status: "Error", 
                error: geminiResult.error || "Gemini API failed to return text." 
            });
        }
        ...
        try {
            ...
            // CRITICAL: Ensure parsedData has a FormattedAddress even if it's null/empty from AI
             if (!parsedData.FormattedAddress) {
                remarks.push("CRITICAL_ALERT: AI returned a null FormattedAddress.");
            }
        } catch (e) {
